  ...
  try {
    ...
    if (!request.headers.get("user-agent").includes("Willow")) {
      ...
      const combinedBuffer = new Uint8Array(
        wavHeader.byteLength + audioData.byteLength
      );
    ...
    return new Response("Error processing the file. - " + error.message, {
      status: error.statusCode || 500,
    });
  }
});

router.get("/api/tts", async (request, env) => {
  try {
...
router.get("/", () => {
  return new Response(
    "Willow Speech Services - Available endpoints: POST /api/willow (STT), GET /api/tts?text=hello (TTS)",
    {
      headers: { "Content-Type": "text/plain" },
    }
  );
